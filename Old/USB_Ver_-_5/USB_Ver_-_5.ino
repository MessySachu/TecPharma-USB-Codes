...
const int chipSelect = 47;  //Chip select for SD card
  byte present = 0;
  byte type_s;
  byte data[12];
  byte addr[8];
  ...
  int CurrentTimePreviousSecond = 0;
...
void serialEvent1(){
    if(Serial1.available()){
       InputDataString = Serial1.readString();
...
//       else{
//       String RFID_Tags;
//          File DataLogFile = SD.open("RFID.txt");
//          if(DataLogFile){
//              while(DataLogFile.available()){
...
void ProcessIncomingData(){
      if(Serial2.available()){
             RecievingData = Serial2.readString();
//             Serial2.println(RecievingData);
             Serial.println(RecievingData);
      }
      if(RecievingData.startsWith("@,00001")){
/*           Serial.println("Device Data Updated;");
           Serial2.println("Device Data Updated;");
           String UpdateDetails = "";
           UpdateDetails += DeviceName;
           UpdateDetails += ",";
           DeviceLevel = RecievingData.substring(8,9).toInt();
       ...
       else if(RecievingData.startsWith("O")){
           OpenDoor();
       }
       else if(RecievingData.startsWith("X")){
           DontOpenDoor();
       }
       else if(RecievingData.startsWith("N")){
           NotRegistered();
       }
       else if(RecievingData.startsWith("&")){
//           LogRFIDCard(RecievingData[1]);
       }
       else if(RecievingData.startsWith("?")){
           UpdateDataInSDCard();   
       }
       else if(RecievingData.startsWith("H")){
           DeviceIdentifyRequested();
       }
       else if(RecievingData.startsWith("^,00001")){
           SystemOnline = true;
       }   
}
...
//  unsigned long CurrentMillisCount = millis();
  delay(200);
//  while(!SystemOnline && ((millis() - CurrentMillisCount) < 5000));
  if(SystemOnline){
    SystemOnline = false;
    return true;
  }
